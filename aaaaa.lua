...
function Compkiller:_DrawKeybinds(Window: ScreenGui)
	if Compkiller.__KEYBINDS_CACHE then
		-- FIX: Valida se o cache ainda é válido (Root existe e tem Parent)
		-- Evita retornar referência a uma janela já destruída causando "attempt to call a nil value"
		if Compkiller.__KEYBINDS_CACHE.Root and Compkiller.__KEYBINDS_CACHE.Root.Parent then
			return Compkiller.__KEYBINDS_CACHE;
		else
			Compkiller.__KEYBINDS_CACHE = nil;
		end;
	end;
...
function Compkiller:_LoadDropdown(BaseParent: TextButton, Callback: (any) -> any?)
	...
	-- Forward declarations to prevent nil upvalue capture in closures
	local ToggleUI
	...
	ScrollingFrame.BackgroundTransparency = 1.000	ScrollingFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
	...
	-- ToggleUI assignment (closures now correctly capture the reference)
	...
	local function BuildOptionRow(v)
		...
		BlockText.TextXAlignment = Enum.TextXAlignment.Left
		BlockText.AnchorPoint = Vector2.new(0, 0.5)
		BlockText.Position = UDim2.new(0, 8, 0.5, 0)
	...
	-- Activation/Deactivation (Shifts Text.Position instead of Padding)
	local function ActivateRow(OptionRow)
		if OptionRow.IsActive then return end
		OptionRow.IsActive = true
		Compkiller:_Animation(OptionRow.Text, TweenInfo.new(0.2), { Position = UDim2.new(0, 12, 0.5, 0) })
		Compkiller:_Animation(OptionRow.Indicator, TweenInfo.new(0.2), { Size = UDim2.new(0, 3, 0, 14), BackgroundTransparency = 0 })
	...
	local function DeactivateRow(OptionRow)
		...
		Compkiller:_Animation(OptionRow.Text, TweenInfo.new(0.2), { Position = UDim2.new(0, 8, 0.5, 0) })
		Compkiller:_Animation(OptionRow.Indicator, TweenInfo.new(0.2), { Size = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1 })
	...
	-- UpdateDropdown assignment with circular reference guard
				...
				if IsMulti then
					if Row.IsActive then
						...
						for idx, val in ipairs(SelectedValues) do
							if val == v then table.remove(SelectedValues, idx); break end						end
					else
						ActivateRow(Row)
						table.insert(SelectedValues, v)
					end
...
function Compkiller.new(Config : Window)
	...
	Compkiller.WindowOpenSignals[CompKiller] = WindowOpen;
	...
	do
		...
		Compkiller:_Input(Userinfo,function()
			-- FIX: Verificação dupla para evitar "attempt to call a nil value" quando :Window ainda não existe
			if WindowArgs.UserSettings.Root and type(WindowArgs.UserSettings.Window) == "function" then
	...
	function WindowArgs:DrawContainerTab(TabConfig : ContainerTab)
		...
		TabArgs:DrawTab(TabConfig : TabConfig) -- Internal Tab
			...
			local UICorner_2 = Instance.new("UICorner")
			local TextLabel = Instance.new("TextLabel")
			local Icon = Instance.new("ImageLabel")   -- ADICIONAR AQUI
			Frame.Parent = Top
			...
			Icon.Image = Compkiller:_GetIcon(TabConfig.Icon or "eye")
			Icon.ImageColor3 = Compkiller.Colors.Highlight
			...
			TextLabel.BorderColor3 = Color3.fromRGB(0, 0, 0)
			TextLabel.BorderSizePixel = 0
			TextLabel.Position = UDim2.new(0, 27, 0.5, 0)
			...
			local UpdateScale = function()
				...
				Frame.Size = UDim2.new(0, scale.X + 36, 0, 26)
			end;
			...
			local ToggleUI = function(bool)
				...
				if bool then
					...
					Compkiller:_Animation(Icon,TweenInfo.new(0.2),{
						ImageTransparency = 0
					})
	...
	function WindowArgs:DrawConfig(Configuration : TabConfigManager , Internal)
		...
		function TabArgs:_DrawConfig()
			...
			-- FIX: Callbacks definidos como nil ANTES dos inputs causavam "attempt to call a nil value"
			-- Agora inicializamos com funções vazias e adicionamos verificação de segurança
			ConfigButton.OnLoad = nil;
			ConfigButton.OnSave = nil;
			ConfigButton.OnDelete = nil;

			Compkiller:_Input(LoadButton,function()
				if ConfigButton.OnLoad and type(ConfigButton.OnLoad) == "function" then
					task.spawn(ConfigButton.OnLoad);
				end
			end);

			Compkiller:_Input(SaveButton,function()
				if ConfigButton.OnSave and type(ConfigButton.OnSave) == "function" then
					task.spawn(ConfigButton.OnSave);
				end
			end);

			DelButton.MouseButton1Click:Connect(function()
				if ConfigButton.OnDelete and type(ConfigButton.OnDelete) == "function" then
